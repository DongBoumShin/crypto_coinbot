...
def calcRSI(df, period=14):  # returns series
    date_index = df.index
...
def adx(df, period=14):
    ...
    direction['dmup'] = df['high']-df['high'].shift(1)
    direction['dmdown'] = df['low']-df['low'].shift(1)
    direction['diup'] = direction['dmup'].ewm(span=period).mean()*100/atrdx
    direction['didown'] = direction['dmdown'].ewm(span=period).mean()*100/atrdx
    direction['dx'] = (direction['diup']-direction['didown'])*100/(direction['diup']+direction['didown'])
    adxnow = direction['dx'].ewm(span=period).mean()
    return adxnow
